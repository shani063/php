...
// Always keep this logic at last so that any variable assigned in CF has the latest value.
$CF = $gh->get_session("all_data");

// prevent any error on logout page due to session not set or any other warnings
if (!empty($CF)) {
	$CF['const'] = $const;
	$CF['current_page'] = $current_page;
}
// Manipulate the string so that it loads with "defer" and then inject the variable in js to be used in handlebards file

// $CF['include_javscript_at_bottom'] = $include_javscript_at_bottom;
$include_javscript_library_before_custom_script_at_bottom .= '<script>
	...
	let CF = ' . print_r(json_encode($CF, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE), true) . '
</script>';
