...
function manage_price() {
    ...
    var disc = ((selected_verient.selling_price * 100) / selected_verient.mrp).toFixed(0);
    $(".discount").html(disc + "% off");
    $(".product-details").html(selected_verient.fetaures);
}

function buyNow() {
    localStorage.setItem("selected_verient", JSON.stringify(selected_verient));
    window.location.href = MAIN_URL + "address";
}